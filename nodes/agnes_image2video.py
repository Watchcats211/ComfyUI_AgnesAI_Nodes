"""
Agnes_Image2Video Node - Image to Video generation via Agnes.ai API

Accepts a URL STRING (from Agnes_Text2Image's url output, or manually pasted).
Outputs:
  - IMAGE: frame batch tensor [N, H, W, C] for VHS_VideoCombine
  - AUDIO: audio dict for VHS_VideoCombine
  - STRING: video URL (for downstream connections)

Workflow:
  Agnes_Config → Agnes_Text2Image → [url output] → Agnes_Image2Video → VHS_VideoCombine
"""
...
class Agnes_Image2Video:
    """
    Generate videos from an image URL using Agnes.ai API.
    Outputs IMAGE tensor batch + AUDIO for VHS_VideoCombine, plus URL for downstream.
    """
        ...
        return {
            "required": {
                ...
                "image_url": ("STRING", {
                    "default": "",
                    "multiline": True,
                    "tooltip": "Public image URL. Connect from Agnes_Text2Image's 'url' output, or paste a URL."
                }),
                "prompt": ("STRING", {
                    "default": "The scene comes to life with gentle motion, clouds drifting, water flowing",
                    "multiline": True,
                    "tooltip": "Describe the motion you want in the video"
                }),
    ...
    def generate(self, config, image_url, prompt, duration, frame_rate,
        ...
        if not image_url or not image_url.startswith("http"):
            print(
                "[Agnes] Image-to-Video skipped: image_url is empty or invalid. "
                "Connect Agnes_ImageToURL or another public URL source."
            )
        ...
        print(f"[Agnes] Image-to-Video: {prompt[:50]}...")
        print(f"[Agnes] ~{dur_sec:.1f}s ({num_frames}f @ {frame_rate}fps)")
        print(f"[Agnes] Source: {image_url[:80]}...")
    ...
    def _download(self, url, task_id):
        try:
            ...
            filename = f"agnes_i2v_{task_id[:12]}.mp4"
